...
app.post("/uploadVideo", async (req, res) => {
  ...
  let videoBase64 = [];
  if (req.body.video) {
    for (let i = 0; i < req.body.fragment; i++) {
      ffmpeg(req.body.video)
        .setStartTime(`00:00:${req.body.time * i}`)
        .setDuration(req.body.time)
        .output(`./video/out${i}.mp4`)
        .on("end", function (err) {
          if (!err) {
            console.log("conversion Done");
            videoBase64.push(
              new Promise((res, rej) => {
                fs.readFile(`./video/out${i}.mp4`, (err, data) => {
        ...
        .on("end", function (err) {
          if (!err) {
            if (+videoBase64.length === +req.body.fragment) {
                console.log('doneeee')
              Promise.all(videoBase64).then((data) => {
                return res.json({ message: "Fragment Video", base64: data });
              });
            }
          }
        })
