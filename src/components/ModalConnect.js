...
import { AiFillCheckCircle, AiOutlinePoweroff } from "react-icons/ai";
...
const StyledModal = styled.div`
  ...
  .modalContainer {
    ...
    right: calc(50% - 233px);
    top: 5%;
    transform: color 0.15s ease-in-out 0s, background-color 0.15s ease-in-out 0s,
    ...
    if (!user || !account) {
      ...
      if (network === "rinkeby") {
        await axios
          ...
          .then((value) => {
            console.log("rinkeby", value.data.balance);
        ...
        await axios
          ...
          .then((value) => {
            console.log("bsc", value.data.balance);
            dispatch(connectMetaMask(accounts, value.data.balance));
          });
      }
    }
