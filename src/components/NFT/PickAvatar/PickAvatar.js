...
export default function PickAvatar() {
  ...
  const createNFT = async (nameDino) => {
    let composite = "";
    if (loading) {
    ...
    if (account && !loading && nameDino.length < 3) {
      toast.warn("Vui lòng nhập tên NFT!!!");
    }

    if (account && nameDino.length >= 3 && !loading) {
      let index = [];
      if (network === "rinkeby") {
        index = await contractEth.methods.currentIndex().call();
      } else {
        index = await contractBsc.methods.currentIndex().call();
      }
      setLoading(true);
      let result = {
        currentBody,
      ...
      console.log("backgroundIpfs", backgroundIpfs);
      if (backgroundIpfs.status) {
        ...
        console.log("fileImage", fileImage, backgroundIpfs);
